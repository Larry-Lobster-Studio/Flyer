...
import type { ListExpenses200ItemsItem } from '@/lib/api/interfaces';
...
interface Props {
	tableData: ListExpenses200ItemsItem[];
	totalItems: number;
	pageCount: number;
...
export function ExpensesTable({
	...
	const [columns, data] = useMemo(() => {
		const columns: ColumnDef<ListExpenses200ItemsItem>[] = [
			{
				accessorKey: 'contact.name',
				cell: (info) => {
					return (
						<Link
							href={`/expenses/${info.row.original.id}`}
							className={clsx('')}
						>
